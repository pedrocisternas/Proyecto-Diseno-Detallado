...
public class Deck
{
    ...
    private void LoadSuperstarCard(string superstarName)
    {
        ...
        SuperstarCard = allSuperstars.FirstOrDefault(s => s.Name.Equals(superstarName, StringComparison.OrdinalIgnoreCase));
    }
    ...
    private void LoadNormalCards(List<string> lines)
    {
        ...
        {
            NormalCard card = allNormalCards.FirstOrDefault(c => c.Title.Equals(cardName, StringComparison.OrdinalIgnoreCase));
            if (card != null)
            {
    ...
    public bool IsValid()
    {
        ...
        return deckValidator.Validate(SuperstarCard, NormalCards);
    }
}
